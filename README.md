...
>
> 16:9 横版 | 小猫 IP | 纯白手绘 | 少量红橙蓝中文批注 | Codex Skill
...
默认视觉 IP 是“小猫”：一只圆脸、蓝灰短毛、琥珀眼、戴红色小龙虾钳子毛绒发箍的母蓝猫。小猫不是普通宠物插画，也不是随手贴上的萌宠贴纸，而是正在认真参与系统运转的幽默行动者。
...
```bash
git clone https://github.com/wkk1994/ian-xiaohei-illustrations.git
...
```text
Use $ian-xiaohei-illustrations 为这篇中文文章设计并生成 5 张小猫怪诞正文配图。
```
...
每张图写清楚：放在哪段后、主题、核心意思、结构类型、小猫在做什么、建议中文标注词。
...
```text
Use $ian-xiaohei-illustrations 把下面这篇文章生成 4 张小猫怪诞正文配图。
...
6. 让小猫承担核心动作
7. 每张图单独调用图像模型生成
8. 按 QA checklist 检查：白底、留白、小猫动作、中文标注、非 PPT 感、非旧案例复刻
9. 保存最终 PNG，并报告用途和路径
...
- 小猫必须承担核心动作；如果去掉小猫画面仍然完全成立，说明小猫太装饰了。
...
这套小猫配图 Skill，只是我用 AI 搭建个人生产系统里的一个小工具。
